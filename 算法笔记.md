...
```java
need.get(c).equals(window.get(c))
```

因为Integer超过127会创建新的对象而不是缓存！！！！！！！！！！！！！
...
![image-20241008160616146](算法笔记.assets/image-20241008160616146.png)

```java
class Solution {
    public int maxCoins(int[] nums) {
        int n = nums.length;
        int[] nums_ = new int[n+2];     // 加上边界的数组，统一处理
        nums_[0] = nums_[n+1] = 1;
        for(int i = 0; i < n; i++){
            nums_[i + 1] = nums[i]; 
        }
        int[][] dp = new int[n+2][n+2]; // dp[i][j]表示数组范围nums_(i, j)可以获得的最大硬币数
        ...
        for(int i = n-1; i >= 0; i--){
            ...
            for(int j = i+2; j < n + 2; j++){
                ...
                for(int k = i + 1; k < j; k++){
                    ...
                    dp[i][j] = Math.max(dp[i][j], dp[i][k] + nums_[i] * nums_[k] * nums_[j] + dp[k][j]);
                }
            }
        }
        return dp[0][n+1];
...
![image-20241008165250143](算法笔记.assets/image-20241008165250143.png)
...
class Solution {
    ...
    void dfs(int u, String cur, int l, int r, int score) {
        ...
        char c = s.charAt(u);
        if (c == '(') {
            dfs(u + 1, cur + String.valueOf(c), l, r, score + 1);//要当前左括号
            dfs(u + 1, cur, l - 1, r, score);//不要当前左括号
        } else if (c == ')') {
            dfs(u + 1, cur + String.valueOf(c), l, r, score - 1);//要当前右括号
            dfs(u + 1, cur, l, r - 1, score);//不要当前右括号
        } else {
            dfs(u + 1, cur + String.valueOf(c), l, r, score);//普通字母
        }
    }
}
```
...
![image-20241008173354032](算法笔记.assets/image-20241008173354032.png)
...
class Solution {
    public int kthSmallest(int[][] matrix, int k) {
        ...
        while(left < right){//最终左右相等，则设定边界可取
            ...
            if(cnt >= k)//mid >= target，则mid作为右边界(包括)
                right = mid;
            else
                left = mid+1;//mid < target，则mid + 1
        }
        return left;
    }
    //计算小于mid的数有几个
...
```java
二分法中，右边界更新后肯定不小于x，即right >= x。问题在于左边界更新后会不会直接越过x，不会，因为left更新前的mid必然是小于x的，所以left = mid + 1必然小于等于x。当循环退出时，right == left。其中right >= x，left <= x，所以其交集必然是x。
```
...
当right == left时，[right, left]中只有一个元素了(即left)，所以此时left必然 ”在矩阵中“ 且 ”满足第k小“
```

## [287. 寻找重复数](https://leetcode.cn/problems/find-the-duplicate-number/)

![image-20241011211327562](算法笔记.assets/image-20241011211327562.png)
...
![image-20241011211936066](算法笔记.assets/image-20241011211936066.png)
...
![image-20241011214914042](算法笔记.assets/image-20241011214914042.png)
...
![image-20241011221606881](算法笔记.assets/image-20241011221606881.png)
...
![image-20241011224003332](算法笔记.assets/image-20241011224003332.png)

```java
class Solution {
    public List<List<String>> groupAnagrams(String[] strs) {
        // 创建一个哈希映射，键为排序后的字符串，值为包含原字符串的列表
        ...
        for (String str : strs) {
            ...
            // 根据键从哈希映射中获取对应的列表，如果键不存在，则创建一个新的空列表
...
![image-20241012164143485](算法笔记.assets/image-20241012164143485.png)
...
![image-20241012165807526](算法笔记.assets/image-20241012165807526.png)
...
![image-20241012200521469](算法笔记.assets/image-20241012200521469.png)
...
![image-20241012203649497](算法笔记.assets/image-20241012203649497.png)
...
![image-20241012210941292](算法笔记.assets/image-20241012210941292.png)
...
//遍历整个字符串s，把所有的合法括号序列按照右括号来分类，对于每一个右括号，都去求一下以这个右括号为右端点的最长的合法括号序列的左端点在什么位置。我们把每个右括号都枚举一遍之后，再取一个max，就是整个的最大长度。
```

## [31. 下一个排列](https://leetcode.cn/problems/next-permutation/)

![image-20241012222554792](算法笔记.assets/image-20241012222554792.png)
...
![image-20241012224557200](算法笔记.assets/image-20241012224557200.png)
...
![image-20241012225156400](算法笔记.assets/image-20241012225156400.png)
...
![image-20241012233624448](算法笔记.assets/image-20241012233624448.png)
...
![image-20241012235045281](算法笔记.assets/image-20241012235045281.png)
...
![image-20241013145316423](算法笔记.assets/image-20241013145316423.png)
...
![image-20241013154830426](算法笔记.assets/image-20241013154830426.png)
...
![image-20241013161105840](算法笔记.assets/image-20241013161105840.png)
...
![image-20241013164333633](算法笔记.assets/image-20241013164333633.png)
...
![image-20241013232050509](算法笔记.assets/image-20241013232050509.png)
...
![image-20241014144642194](算法笔记.assets/image-20241014144642194.png)
